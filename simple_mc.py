...
"""Important note: the calculations are done on the Excel spreadsheet, 
but the collection of inputs and outputs is done by the program using
 dataframes.  For certain calculations involing series of periods 
 (e.g. a financial model) this may be the most practical way to do the calculations.  
 However, for large #s of simluations, this will be slow so in some cases, 
 it may be best to move the calculations to the program, while still exporting 
 summary stats and plots to the worksheet."""
...
results.range('D1').value = description
